# ChatGPT Local Files — the agent map
...
App **1.8.9** (`package.json`, `src/main/version.ts`) · extension **1.8.9**
(`extension/manifest.json`) · bridge protocol **6** (`version.ts::BRIDGE_PROTOCOL`).
Tests assert those stay in sync. Windows-first; main process is TypeScript; extension is
plain MV3 JavaScript with no build step; Vitest; `node-pty` is the one native dependency.

Defaults from `config.ts` — **read-only on**, **recording on**, session advisory/limit
**300k/400k** estimated tokens, **auto-compaction on at 300k and edge-triggered**,
**multi-agent off** with `maxWorkers` 2 (hard max 8). Existing configs keep explicit user
choices; migrations only move untouched old defaults.
...
test/*.test.ts                36 suites, named for the subsystem they cover
bughunt-*/                    retained forensic reports and adversarial repros
...
**Core** (`chatgpt-local-files-core`, required):
...
**Desktop** (`chatgpt-local-files-desktop`, optional): `observe` needs `screen`;
...
- Canonicalize-then-open is a TOCTOU surface. A fix must close the race, not add a
  third pre-check.
...
```text
%APPDATA%\chatgpt-local-files\sessions\<id>\
...
Experimental, off by default, **one global run at a time**, star topology:
...
argument**. `join` is an exceptional, user-mediated recovery path for a worker whose
automatic binding was lost.
...
**On-disk state to inspect.** `%APPDATA%\chatgpt-local-files\` — `config.json` (non-secret
...
| agents spawn/join/message/finish | `agents.ts`, `tools-core.ts`, `bridge.ts` | `agents`, `swarm`, `bridge` |
...
Workers are already bound to their slot when launched; `join` is recovery only.
...
The user often reports recurring symptoms from memory rather than a clean issue list. In
that case recent transcripts, durable session metadata and retained `bughunt-*` files are
first-class input, not optional reading. Follow **one** concrete request id, conversation
id, worker slot or event sequence end to end.

## 20. Packaging and release — `electron-builder.yml`

App id `com.chatgptlocalfiles.app`, product `ChatGPT Local Files`, Windows x64 NSIS,
per-user, `asInvoker`, no elevation.
...
- `node-pty` is **not** rebuilt and is unpacked from asar, because ConPTY assets must be
  real files on disk.
- Uninstall deliberately preserves `%APPDATA%\chatgpt-local-files`.
...
- **Request identity / Fiber health** — a bound worker keeps executing while its MCP calls
  ...
  MAIN-world Fiber; `content.js` can remember Fiber "once existed" and suppress the degraded
  DOM fallback; correlation restore can trust a stale snapshot over newer session history.
  Cascades into false stalls, broker failures, workspace/terminal uncertainty.
- **Browser lifecycle** — provisional fresh-chat observations, delayed dying-document IPC,
  retries and worker restarts can lose, duplicate or misbind transcript state.
- **Filesystem** — validated pathname strings still carry reparse/junction TOCTOU risk;
  native traversal and error text must match the virtual contract.
- **MCP / process lifetime** — accepted mutation vs. client disconnect, bridge startup
  serialization, terminal exit races, structured output caps.
