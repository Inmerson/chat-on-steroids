...
export async function enrollRemote(friendlyName: string, capabilities: string[]): Promise<ManagedDeviceSummary> {
  const saved = await readDurable<unknown>(REGISTRY_STATE);
  const state: RegistryState = validRegistry(saved) ? saved : { version: 1, local: localSummary(), remotes: [] };
  const remote: ManagedDeviceSummary = {
    deviceId: `dev_${randomUUID().replaceAll('-', '')}`,
    friendlyName: friendlyName.trim().slice(0, 80) || 'Remote computer',
    provider: 'remote',
    status: 'ONLINE',
    capabilities: [...new Set(capabilities)].slice(0, 16),
...
export async function enrollRemoteWithResume(friendlyName: string, capabilities: string[]): Promise<RemoteEnrollment> {
  ...
  await writeDurableNow(REMOTE_RESUME_STATE, {
    version: 1,
    credentials: [
      ...credentials.filter((entry) => entry.deviceId !== device.deviceId),
      { deviceId: device.deviceId, tokenDigest: digest(resumeToken) }
    ]
...
export async function setRemotePresence(
  deviceId: string,
  status: Extract<ManagedDeviceSummary['status'], 'ONLINE' | 'OFFLINE' | 'DEGRADED'>
): Promise<void> {
  const saved = await readDurable<RegistryState>(REGISTRY_STATE);
...
export async function revokeRemote(deviceId: string): Promise<boolean> {
  ...
  const remotes = saved.remotes.map((entry) =>
    entry.deviceId === deviceId ? { ...entry, status: 'REVOKED' as const, lastSeenAt: new Date().toISOString() } : entry
  );
