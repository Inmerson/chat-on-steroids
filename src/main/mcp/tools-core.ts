        ...
        inputSchema: z
          .object({
            ...
            device_id: z
              ...
              .regex(/^[A-Za-z0-9_-]+$/)
              .optional()
              .describe('Explicit managed device ID. Omit for this computer.')
          })
          .strict(),
        ...
        guard('read', async () => {
          ...
          if (device_id) {
            ...
            try {
              ...
              return fail(error instanceof Error ? error.message : 'Remote read failed');
            }
          }
        ...
        guard('apply_patch', async () => {
          ...
          if (device_id) {
            ...
            try {
              ...
              return fail(error instanceof Error ? error.message : 'Remote patch failed');
            }
          }
    ...
    reg.register(
      ...
      async (input) =>
        reg.guarded('command', 'exec_command', async () => {
          if (input.device_id) {
            if (!ctx.multiDevice) return fail('DEVICE_NOT_FOUND: multi-device transport is unavailable');
            if (input.cmd === undefined || input.tty === true) {
              return fail('INVALID_REQUEST: remote exec requires one non-interactive cmd');
            }
            try {
              const result = await ctx.multiDevice.execute(input.device_id, 'terminal.exec', {
                command: input.cmd,
                cwd: input.workdir ?? ctx.roots[0]?.path,
                tty: false
              });
              ...
              const output: ExecCommandToolOutput = {
                chunkId: '',
                wallTimeMs: 0,
                rawOutput: Buffer.from(text, 'utf8'),
              ...
              return fail(error instanceof Error ? error.message : 'Remote command failed');
            }
          }
