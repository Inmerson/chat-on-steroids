...
/**
 * One tool, five actions, registered only while multi-agent mode is on. Fresh installs enable
...
function registerAgentsTool(reg: SurfaceRegistrar): void {
  reg.register(
    'agents',
    {
      ...
      description:
        'Coordinate ChatGPT workers or run a fast read-only Antigravity Flash investigation. ' +
        'spawn creates worker chats; message routes corrections/results; status reports the run; finish is worker-terminal; ' +
        'investigate scans an approved workspace and returns advisory evidence that the prime must verify.',
      inputSchema: z.object({
        ...
        context: z
          ...
          .max(4000)
          .optional()
          .describe('spawn: shared repo, constraints and validation instructions supplied once to every worker.'),
        workers: z
          .array(
            z.object({
              label: z.string().max(60).optional().describe('Short worker label.'),
              task: z.string().min(1).max(4000).describe('spawn: this worker\'s specific task, scope and handoff requirements.')
            })
          )
          .min(1)
          .max(8)
          .optional()
          .describe('spawn: workers to create.'),
        messages: z
          .array(
            z.object({
          ...
          .max(16)
          .optional()
          .describe('message: atomic batch; prefer this for several recipients.'),
        to: z.string().min(1).max(40).optional().describe('message: one recipient.'),
        text: z.string().min(1).max(4000).optional().describe('message: text for one recipient.'),
        result: z
          .string()
          .min(1)
          .max(4000)
          .optional()
          .describe('finish: factual RESULT / CHANGES / VALIDATION / BLOCKERS handoff to the prime.'),
        task: z
          .string()
          .min(1)
          .max(4000)
          .optional()
          .describe('investigate: narrow read-only reconnaissance question; never final verification.'),
        workdir: pathArg
          .optional()
          .describe('investigate: approved workspace; defaults to current workspace or first root.')
      }),
