...
const store = new AsyncLocalStorage<{ requestId: string | null; timing?: InboundTiming; publication?: unknown }>();
...
export function formatInboundTiming(timing: InboundTiming): string {
  ...
  return (
    ` calls=${timing.calls} ingress_ms=${ms((timing.dispatchedAt ?? timing.startedAt) - timing.startedAt)}` +
    Object.entries(timing.phases)
      .map(([phase, value]) => ` ${phase}_ms=${ms(value)}`)
      .join('') +
...
export function inboundPublication(): unknown | undefined {
