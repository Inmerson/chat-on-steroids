...
/** Named failure breakdown; zero categories are omitted so clean rows stay short. */
 ...
 * The cursor codec.
 *
 * A cursor is retyped by the model on every continuation, and long opaque base64 JSON strings
 * are easy to corrupt while being copied. Keep the token short and made of predictable fields,
 * then bind it to its search/session scope with a checksum so a damaged or foreign copy fails
 * explicitly instead of looking like stale history.
 */
...
/** Decode one caller-supplied cursor for a search or session scope. */
function decodeCursor(raw: string, scope: string): { cursor: SessionCursor } | { error: string } {
  ...
  if (!split) return { error: `Not a session cursor. ${CURSOR_REPAIR_HINT}` };
