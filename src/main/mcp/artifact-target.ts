...
export interface ArtifactTargetOptions { parentReal: string; rootReal: string; name: string; maxFileBytes: number; }
...
export async function openArtifactTarget({ parentReal, rootReal, name, maxFileBytes }: ArtifactTargetOptions): Promise<ArtifactTarget> {
  ...
  async function verifyParent(): Promise<void> {
    ...
    if (!current.isDirectory() || current.isSymbolicLink() || !sameFile(current, parent) ||
  ...
  async function verifyPartial(): Promise<void> {
    ...
    if (!fd.isFile() || !current.isFile() || current.isSymbolicLink() || !sameFile(fd, original) || !sameFile(current, original) ||
        fd.size !== writtenBytes || current.size !== writtenBytes) throw new ArtifactTargetError('Artifact partial changed before publication.');
  }
  async function close(): Promise<void> {
    if (closed) return;
    closed = true;
    try { await verifyParent(); if (await ownedFile(partialPath)) await fs.unlink(partialPath); } catch { /* leave foreign state untouched */ }
    await handle.close();
  }
  try { await verifyPartial(); } catch (error) { await close().catch(() => undefined); throw error; }
  return {
    ...
    async syncAndVerify(expectedSize) {
      ...
      await handle.sync(); await verifyPartial(); verified = true;
    },
    async publish() {
