/**
 * Bounded Code Mode host adapted from upstream v2.0.9. Legal/provenance details are retained in
 * docs/licenses/codex. The host owns limits and validation; model JavaScript owns no Node authority.
 */
...
export const codeModeSchema = z
  .object({
    code: z
      .string()
      .min(1)
      .max(CODE_MODE_LIMITS.codeChars)
      .describe('Raw JavaScript source with top-level await. Emit results with text(...) or image(...).')
  })
  .strict();
...
async function emittedImage(value: unknown): Promise<ToolContent> {
  ...
  if (
    typeof data !== 'string' ||
    typeof mime !== 'string' ||
    data.length > Math.ceil(MAX_VIEW_IMAGE_BYTES / 3) * 4 ||
    data.length % 4 !== 0 ||
...
/**
 * A fresh interpreter per call. Accepted nested operations retain their dispatcher lifetime after
 * interpreter termination; this owner never claims a filesystem/process action was rolled back.
 */
export async function runCodeMode(
  ...
  const errorResult = (reason: string): ToolResult => ({
    content: [{ type: 'text', text: `CODE_MODE_${reason}` }],
    isError: true
  });
  if (!code.trim() || code.length > limits.codeChars) return errorResult('CODE_LIMIT');
  if (activeRuns >= limits.activeRuns) {
    return errorResult('BUSY: too many code executions or unsettled nested calls.');
  }
  if (/^\s*\/\/ @exec:/m.test(code)) {
    return errorResult('UNSUPPORTED_PRAGMA: use plain JavaScript; execution is bounded to one call.');
  }
  ...
  try {
    const status = await new Promise<string | null>((resolve) => {
      ...
      worker.on('exit', (code) => {
        if (!ended) finish(code === 0 ? null : 'RUNTIME_ERROR');
      });
      worker.on('message', (message: { type: string; id?: number; json?: string; kind?: string; error?: string | null }) => {
        ...
        if (message.type === 'emit') {
          if (
            ...
            emissions.length >= limits.outputItems ||
            bytes > limits.resultBytes ||
            (emittedBytes += bytes) > limits.outputBytes
          ) {
            finish('OUTPUT_LIMIT');
            return;
          }
          if (
            message.kind === 'text'
              ? (textBytes += bytes) > limits.textBytes
        ...
        if (
          ...
          calls >= limits.calls ||
          pending.size >= limits.concurrentCalls ||
          bytes > limits.argumentBytes
        ) {
          finish('CALL_LIMIT');
          return;
        }
        ...
        try {
          request = JSON.parse(message.json);
        } catch {
          finish('BRIDGE_ERROR');
          return;
        }
        ...
        const work = Promise.resolve()
          .then(() => invoke(request.name, request.args))
          .then(
            (result) => {
              if (ended) return;
              const json = JSON.stringify(result);
              const bytes = Buffer.byteLength(json);
              if (bytes > limits.resultBytes || (resultBytes += bytes) > limits.totalResultBytes) {
                finish('RESULT_LIMIT');
                return;
              }
            ...
            () => {
              if (!ended) {
                worker!.postMessage({ type: 'result', id, json: JSON.stringify(errorResult('TOOL_ERROR')) });
              }
            }
          )
    ...
    for (const emission of emissions) {
      try {
        ...
        if (emission.kind === 'text' && typeof value === 'string') {
          content.push({ type: 'text', text: value });
        } else if (emission.kind === 'image') {
          content.push(await emittedImage(value));
        } else {
          throw new Error('OUTPUT_INVALID');
        }
      } catch {
        return {
          content: [...content, ...errorResult('OUTPUT_INVALID: image or text could not be validated.').content],
          isError: true
        };
      }
    }
    if (status) {
      ...
      content.push(
        ...errorResult(`${status}: execution stopped. ${effects} Unemitted values remain private.`).content
      );
    }
    if (pending.size) {
      content.push(
        ...errorResult(
          'UNAWAITED_CALLS: dispatched tool calls are still running and remain recorded. Side effects were not cancelled.'
        ).content
      );
    }
    ...
    if (pending.size) {
      void Promise.allSettled([...pending]).then(() => { activeRuns--; });
    } else {
      activeRuns--;
    }
  }
