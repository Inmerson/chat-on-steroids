...
export async function ensureCoreIpcToken(userDataDir: string): Promise<string> {
  ...
  if (!/^[0-9a-f]{64}$/i.test(existing)) throw new Error('Core IPC authentication token is corrupt');
...
async function dispatch(request: CoreRequest, handlers: CoreIpcHandlers): Promise<unknown> {
  switch (request.command) {
    case 'hello': return handlers.hello();
    case 'status': return handlers.status();
    case 'connect': await handlers.connect(); return handlers.status();
    case 'disconnect': await handlers.disconnect(); return handlers.status();
    case 'apply-settings': await handlers.applySettings(); return handlers.status();
    case 'secret-status': return handlers.secretStatus();
    case 'set-secret':
    ...
    case 'shutdown-core': await handlers.shutdownCore(); return true;
  }
}
...
function handleSocket(socket: Socket, handlers: CoreIpcHandlers): void {
  ...
  socket.on('data', (chunk: Buffer) => {
    ...
    if (Buffer.byteLength(buffered) > MAX_IPC_REQUEST_BYTES) {
    ...
    void Promise.resolve(dispatch(request, handlers)).then(
      ...
      (error: unknown) => response(socket, { id: request.id, ok: false, error: error instanceof Error ? error.message : String(error) })
    );
  });
}
...
export async function startCoreIpcServer(handlers: CoreIpcHandlers): Promise<CoreIpcServer> {
  ...
  await new Promise<void>((resolve, reject) => {
    const onError = (error: NodeJS.ErrnoException): void => {
      ...
      reject(error.code === 'EADDRINUSE' ? new Error('Core endpoint is already owned by another process') : error);
    };
...
export class CoreIpcClient {
  ...
  private async request<T>(command: CoreRequest['command'], extra: Record<string, unknown> = {}): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      socket.on('data', (chunk: Buffer) => {
        buffered += chunk.toString('utf8');
        if (Buffer.byteLength(buffered) > MAX_IPC_RESPONSE_BYTES) {
        ...
        try {
          ...
          if (parsed.id !== id) return finish(new Error('Core IPC response id mismatch'));
          if (!parsed.ok) return finish(new Error(parsed.error || 'Core IPC request failed'));
  ...
  hello(): Promise<CoreHello> { return this.request('hello'); }
  status(): Promise<CoreStatusEnvelope> { return this.request('status'); }
  connect(): Promise<CoreStatusEnvelope> { return this.request('connect'); }
  disconnect(): Promise<CoreStatusEnvelope> { return this.request('disconnect'); }
  applySettings(): Promise<CoreStatusEnvelope> { return this.request('apply-settings'); }
  secretStatus(): Promise<CoreSecretStatus> { return this.request('secret-status'); }
  setSecret(key: CoreSecretKey, value: string): Promise<void> { return this.request('set-secret', { key, value }); }
  uiCall<T>(operation: CoreUiOperation, payload: unknown): Promise<T> { return this.request('ui-call', { operation, payload }); }
  shutdownCore(): Promise<boolean> { return this.request('shutdown-core'); }
}
