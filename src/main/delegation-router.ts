/**
 * Deterministic policy for the Antigravity read-only fast lane.
 *
 * Keep this cheaper than delegation itself: no I/O, model call, network access or durable state.
 */
...
const POSITIVE_SIGNALS: readonly Signal[] = [
  ...
  {
    pattern:
      /\b(repo[- ]wide|codebase|across (?:the )?(?:repo|repository|codebase|multiple files)|multiple files|execution path|request path|call graph|dependency graph|lifecycle)\b/i,
    score: 2,
...
const HARD_BLOCKS: ReadonlyArray<{ pattern: RegExp; reason: string }> = [
  ...
  {
    pattern:
      /^\s*(?:please\s+)?(?:implement|modify|edit|write|patch|fix|change|delete|remove|rename|move|create|add|update|migrate|deploy|publish|release|push|commit|merge|install|uninstall|rotate)\b/i,
    reason: 'mutation, release, or deployment work is not delegated to the read-only investigator'
  },
  {
    pattern:
      /\b(?:and|then)\s+(?:implement|modify|edit|write|patch|fix|change|delete|remove|rename|move|create|add|update|migrate|deploy|publish|release|push|commit|merge|install|uninstall|rotate)\b/i,
...
export function routeAntigravityInvestigation(task: string): DelegationDecision {
  ...
  for (const block of HARD_BLOCKS) {
    ...
    return { delegated: false, score: -100, reasons: [block.reason], hardBlocked: true };
  }
  ...
  return { delegated, score, reasons: reasons.slice(0, 6), hardBlocked: false };
}
