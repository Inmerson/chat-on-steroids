...
export class PluginManager {
  constructor(
    private openAuthorization: (url: URL) => Promise<void> = async () => {
      throw new Error('Plugin authorization UI is not connected.');
    },
  ) {}
  private root = '';
  ...
  private exposure() {
    return pluginExposure(this.records.map(row => ({
      ...
      tools: row.catalog.map(tool => ({ ...tool, description: tool.description ?? '' })), disabledTools: row.disabledTools,
    })));
  }
  ...
  private validateCredentialLaunchTemplates(launch: InstalledLaunch, credentialKeys: string[]): void {
    if (!launch.manifest || credentialKeys.length === 0) return;
    const manifest = vAny.McpbManifestSchema.parse(launch.manifest);
    const base = manifest.server.mcp_config as {
      command?: unknown;
      args?: unknown;
      platform_overrides?: Record<string, { command?: unknown; args?: unknown }>;
    } | undefined;
    const configs = base ? [base, ...Object.values(base.platform_overrides ?? {})] : [];
    for (const key of credentialKeys) {
      const marker = `\${user_config.${key}}`;
      if (configs.some(config =>
        (typeof config.command === 'string' && config.command.includes(marker)) ||
        (Array.isArray(config.args) && config.args.some(arg => typeof arg === 'string' && arg.includes(marker))))) {
        throw new Error('MCPB credentials may not be placed in the subprocess command or arguments; use environment variables.');
      }
    }
  }
  ...
  install(request: PluginInstallRequest): Promise<PluginSnapshot> {
    return this.serial('install', async () => {
      ...
      try {
        ...
        this.validateCredentialLaunchTemplates(launch, Object.keys(request.credentials ?? {}));
  ...
  private async replace(
    ...
    try {
      ...
      if (metadata.fields?.some((f) => f.secret && f.key in config))
        throw new Error('Store sensitive bundle fields in secure credentials');
      this.validateCredentialLaunchTemplates(launch, row.credentialKeys);
      await this.disconnect(row);
  ...
  tools(): Tool[] { return this.exposure().tools.map(tool => ToolSchema.parse(tool)); }
