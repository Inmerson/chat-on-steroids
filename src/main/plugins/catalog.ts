...
/** Reviewed upstream recipes; versions are pinned until an explicit update. */
export const pluginCatalog: PluginCatalogEntry[] = [
  ...
  },
  {
    ...
    source: {
      ...
      dependencies: [{ package: 'mcp', version: '1.30.0' }]
    },
  ...
  },
  {
    ...
    fields: [],
    tools: [
      'Generate raster and vector images',
      'Vectorize images',
...
export function reviewedPluginLicense(source: PluginSource, fallback: string): string {
  return (
    pluginCatalog.find(
      (recipe) =>
        recipe.source.kind === source.kind &&
        recipe.source.package === source.package &&
        recipe.source.version === source.version &&
        (source.kind === 'npm' || source.kind === 'python')
    )?.license ?? fallback
  );
}
