 ...
 */
export const GOAL_SYSTEM_TRAILER = `That was the conversation. Now write the next message as the user: name what they asked for that is still not done, and tell ChatGPT to keep going. Answer exactly NO_REPLY only if everything they asked for is clearly finished and every question of theirs is answered. NO_REPLY is only your completion candidate; Core may continue when durable local evidence contradicts it. Lean towards continuing — a needless "keep going" costs one turn, a wrong stop abandons the job. Write in their language and register, and write nothing except that message.`;

export const GOAL_OBJECTIVE_TRAILER = `That was the conversation. Now write the next message as the user: name the parts of the goal that are still not done and tell ChatGPT to keep going. Answer exactly NO_REPLY only if the goal is completely reached — every part actually done, not planned, promised or described. NO_REPLY is only your completion candidate; Core may continue when durable local evidence contradicts it. Be eager: when in doubt, keep going. Never ask for anything the goal does not ask for. Write in the user's language and register, and write nothing except that message.`;
