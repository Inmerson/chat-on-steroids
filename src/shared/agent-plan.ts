...
/** Codex-style update_plan contract, with bounded per-step detail for the desktop view. */
export const agentPlanUpdateSchema = z
  .object({
    ...
    plan: z
        ...
        z
          .object({
            ...
            details: z
              ...
              .max(2000)
              .optional()
              .describe('Concrete approach, checks or remaining work beneath this headline.')
          })
          .strict()
      )
      .max(12)
      ...
      .refine(
        (steps) => steps.reduce((size, step) => size + step.step.length + (step.details?.length ?? 0), 0) <= 12_000,
        'Keep the complete plan below 12,000 characters.'
      )
