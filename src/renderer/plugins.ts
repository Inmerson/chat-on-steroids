...
function showConnection(): void {
  ...
  actions.append(button('Open ChatGPT plugins', async () => { await run(window.api.openLink('https://chatgpt.com/#settings/Apps')); }), button('Save & connect', async () => {
    if (!appState) return;
    ...
    if (tunnel) {
      const { capabilities, readOnly, tunnel: previousTunnel, ui, sessions, compaction, multiAgent, device, goal } = appState.config;
      const base: SettingsPatch = { capabilities, readOnly, tunnel: previousTunnel, ui, sessions, compaction, multiAgent, device, goal };
...
export function initPlugins(onState: (next: AppState) => void = () => {}): void {
  ...
  $('pluginsOpenChatGPT').addEventListener('click', async () => { await run(window.api.openLink('https://chatgpt.com/#settings/Apps')); });
  $('pluginsLegalOpen').addEventListener('click', async () => { await run(window.api.openPluginLegalNotices()); });
  void refreshPlugins();
}
