...
export function initBrowserPreferences(): void {
  ...
  const request = async (patch: Partial<BrowserPreferences> = {}): Promise<void> => {
    ...
    busy = true;
    paint();
    status.textContent = 'Waiting for the extension to confirm?';
    try {
      const response = await window.api.browserPreferences(patch);
      if (response.ok) {
        confirmed = response.data;
        status.textContent = 'Confirmed by the browser extension.';
      } else {
        confirmed = null;
      ...
      confirmed = null;
      status.textContent = 'Unable to reach the extension. Connect it and refresh.';
    } finally {
      busy = false;
      paint();
    }
