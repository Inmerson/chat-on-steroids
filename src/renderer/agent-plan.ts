...
export function renderAgentPlan(host: HTMLElement, sessionId: string | null, plan: AgentPlan | null): void {
  ...
  const expanded = new Map(
    [...host.querySelectorAll<HTMLDetailsElement>('[data-step]')].map((row) => [row.dataset.step, row.open])
  );
  ...
  heading.append(
    icon('i-steps'),
    el('span', 'agent-plan-title', () => completed === plan.plan.length ? t('Plan complete') : t('Plan')),
  ...
  for (const [index, step] of plan.plan.entries()) {
    ...
    ui(
      marker,
      'aria-label',
      () => step.status === 'in_progress' ? t('In progress') : step.status === 'completed' ? t('Completed') : t('Pending')
    );
    ...
    body.append(row);
    if (focused === step.step) {
      queueMicrotask(() => {
        if (row.isConnected) summary.focus();
      });
    }
  }
  shell.append(body);
    ...
    reduced
      ...
      : [
          ...
          },
          {
            ...
            opacity: 1,
            transform: 'scale(1)',
            boxShadow: 'inset 0 0 28px 0 rgba(70, 210, 150, .25)',
            offset: 0.25
          },
          {
            height: `${height}px`,
