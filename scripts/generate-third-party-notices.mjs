...
const notices = [
  ...
  'runtime notices remain packaged beside their corresponding binaries; this file additionally',
  'preserves the reviewed notices and service references for the optional plugin catalog.',
  '',
...
notices.push(
  '='.repeat(80),
  'OpenAI Codex — adapted coding instructions and update_plan contract',
...
for (const file of ['LICENSE', 'NOTICE']) {
  notices.push(
    `--- Codex ${file} ---`,
    await fs.readFile(path.join(root, 'docs/licenses/codex', file), 'utf8'),
    ''
  );
}
let count = 0;
