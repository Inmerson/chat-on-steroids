 ...
 */
const APP = 'ChatGPT Local Files Core';
const DESKTOP_APP = 'ChatGPT Local Files Desktop';
...
function request(
  ...
  const body = tool === null ? head : `${head},"args":{"note":"${'x'.repeat(80)}","path":"C:/Users/totec/secret.txt"}}`;
  return {
    id,
...
describe('reading a row out of the page', () => {
  ...
  it('names six sequential calls in the same chat, each as itself', async () => {
    ...
    expect(stamps).toEqual(['0', '1', '2', '3', '4', '5']);
  });
...
describe('the calls a turn says it made', () => {
  ...
  /**
   * A name is not a prefix game. `ChatGPT Local Files Backup` shares every character of the
  ...
  it('refuses a connector whose name merely starts with this app’s brand', async () => {
    const messages = [
      request('req-fake', 'read', { app: 'ChatGPT Local Files Backup' }),
      answer('res-fake', 'req-fake', 'read', 'ChatGPT Local Files Backup'),
