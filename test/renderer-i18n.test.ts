...
describe('Chinese app interface', () => {
  ...
  it('switches both ways without replacing controls, icons, emphasis, drafts or authored content', async () => {
    ...
    initLanguage();
    let input = document.getElementById('chatInput') as HTMLTextAreaElement | null;
    if (!input) {
    ...
    let automation = document.getElementById('chatAutomation') as HTMLSelectElement | null;
    if (!automation) {
      automation = document.createElement('select');
      automation.id = 'chatAutomation';
      automation.innerHTML = '<option value="off">Off</option><option value="goal">Goal</option><option value="loop">Loop</option>';
      document.body.append(automation);
    }
  ...
  it('keeps provider model/effort identities and recorded worker markers unchanged in Chinese', async () => {
    ...
    (window as any).api = {
      getChatModels: async () => ({ ok: true, data: { state: 'ready', requestedAt: 1, observedAt: 2, models: [{ id: 'gpt-6-astra', label: 'GPT-6 Astra', efforts: ['high', 'max'], aliases: [] }] } }),
      onChatModelsChanged: (callback: typeof onModels) => { onModels = callback; return () => {}; }
    };
    ...
    expect(list.textContent).toContain('GPT-6 Astra');
