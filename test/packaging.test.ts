...
describe('cross-platform packaging targets', () => {
  ...
  it('assembles every platform artifact in the reusable release workflow', () => {
    ...
    expect(workflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 30s xvfb-run -a /usr/bin/chat-on-steroids');
    ...
    expect(debGui).toContain('--kill-after=5s 30s');
    ...
    expect(appImageGui).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 30s xvfb-run -a "$appimage" >"$log"');
  ...
  it('uses Noble-compatible Linux packages and a FUSE-independent AppImage runtime', () => {
    ...
    expect(releaseWorkflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 30s xvfb-run -a "$appimage" >"$log"');
    expect(releaseWorkflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 30s xvfb-run -a /usr/bin/chat-on-steroids');
