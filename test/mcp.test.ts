...
describe('surface boundaries', () => {
  ...
  it('advertises exactly Desktop’s tools on Desktop, with nothing from Core', async () => {
    ...
    for (const name of surfaceDefinition('core').tools.filter((name) => name !== 'exec')) {
      expect(names, name).not.toContain(name);
    }
  });
...
describe('capability gating', () => {
  it('hides filesystem writing/running tools in read-only mode while keeping execution control available', async () => {
    ...
    expect(toolNames(await core('tools/list'))).toEqual(['exec', 'find', 'read', 'session', 'view_image']);
  });
  ...
  it('keeps only the execution control surface when every reading permission is off', async () => {
    ...
    expect(toolNames(await core('tools/list'))).toEqual(['exec', 'session']);
  });
});
...
describe('the outcome a shell command is recorded with', () => {
  ...
  it('calls a completed non-zero exit the child process outcome, not a connector defect', () => {
  ...
  it('classifies a timeout as an internal tool failure and lets it outrank child status', () => {
