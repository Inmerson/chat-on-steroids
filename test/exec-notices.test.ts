...
vi.mock('../src/main/codex/manager.js', () => ({
  unifiedExecManager: {
    backgroundState: (owned: Set<number>) => ({
      running: [...processes]
        .filter(([id, exit]) => owned.has(id) && exit === null)
        .map(([id]) => id),
      exitedUnread: [...processes]
        .filter(([id, exit]) => owned.has(id) && exit !== null)
        .map(([processId, exitCode]) => ({ processId, exitCode }))
    })
  }
}));
...
function process(id = 10, exit: number | null = null, owner = 'session-a') {
  processes.set(id, exit);
  noteExecOwner(id, owner);
}
