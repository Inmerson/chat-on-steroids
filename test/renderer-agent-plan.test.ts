...
beforeEach(() => {
  dom = new JSDOM('<section id="plan"></section>');
  vi.stubGlobal('document', dom.window.document);
  host = document.getElementById('plan')!;
...
it('celebrates completion once, dismisses the card and keeps completed reloads hidden', async () => {
  const complete = {
    ...plan,
    updatedAt: 2,
    plan: plan.plan.map((step) => ({ ...step, status: 'completed' as const }))
  };
  renderAgentPlan(host, 'a', plan);
...
it('uses a brief fade for reduced motion', () => {
  ...
  expect(dom.window.HTMLElement.prototype.animate).toHaveBeenCalledWith(
    [{ opacity: 1 }, { opacity: 0 }],
