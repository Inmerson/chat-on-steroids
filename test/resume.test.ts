...
describe('the replacement chat', () => {
  ...
  it('leaves the session in chat A when the new chat never revealed its id', async () => {
    ...
    // to, so the ack is refused rather than committed against nothing.
    const acked = await request('POST', '/commands/ack', { body: { id: commandId, status: 'sent' } });
  ...
  it('commits once, so a repeated ack cannot move the session again', async () => {
    ...
    // The command is gone; a duplicate ack is about nothing and changes nothing.
    const repeat = await request('POST', '/commands/ack', {
      body: { id: commandId, status: 'sent', conversationId: 'c-somewhere-else' }
    });
...
describe('a restart in the middle', () => {
  // The transaction is in memory on purpose: it is a promise about one live move. Nothing
  // persists that could be redeemed tomorrow by a chat nobody is waiting for.
  it('does not restore a replacement chat, and leaves the session where it was', async () => {
    await connect();
    const sessionId = await record();
    const { token: continuation } = await press();
    await capture(continuation);
