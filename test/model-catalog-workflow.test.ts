...
it('does not start a fresh native input while model catalog inspection owns provider controls', async () => {
  const start = source.indexOf('  async function runFreshInput(');
  const end = source.indexOf('  function ownsCommandDocument(', start);
  const fresh = source.slice(start, end);
  const context = vm.createContext({
    modelCatalogBusy: true,
    inputAttempt: null,
    inputsHandled: new Set(),
    inputMarkerId: () => nonce
  });
  vm.runInContext(`${fresh}\nglobalThis.runFresh = runFreshInput;`, context);
  await expect((context.runFresh as Function)(nonce)).resolves.toBe(false);
});
