...
const tool = (name: string): PluginToolSchema => ({
  name,
...
const source = (id: string, name: string, names: string[]): PluginExposureSource => ({
  id,
  name,
  enabled: true,
  disabledTools: [],
...
it('publishes exact upstream names and complete schemas without IDs, prefixes or hashes', () => {
  ...
  expect(result.tools.map((entry) => entry.name)).toEqual(['create_entities', 'read_graph', 'get_scene_info']);
...
it('withholds every conflicting name independent of installation order and keeps other tools', () => {
  ...
  for (const sources of [[first, second], [second, first]]) {
    ...
    expect(result.tools.map((entry) => entry.name).sort()).toEqual(['first_only', 'second_only']);
...
it('does not normalize distinct upstream names into aliases and rejects duplicate declarations', () => {
  ...
  expect(result.tools.map((entry) => entry.name)).toEqual(['Echo.Mixed', 'echo_mixed', 'echo-mixed']);
...
it('uses the same bounded publication for discovery and callable ownership', () => {
  const result = pluginExposure([
    source('one', 'Many', Array.from({ length: PLUGIN_MAX_TOOLS + 1 }, (_, index) => `tool_${index}`))
  ]);
