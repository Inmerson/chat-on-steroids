...
| Run executable | `inspect_repo`, `run_command`, `launch_app`, `process_status`, `process`, `open_url` | Narrow read-only Git/process inspection plus direct execution/launch and managed long-running processes; see the warning below |
...
`run_powershell` and `run_command` let ChatGPT **execute code on your PC as you**. They are off by default and should stay off unless you actually need them. For common inspection work, prefer the narrower `inspect_repo` and `process_status` tools: they cannot accept arbitrary shell text and are explicitly read-only. This also gives ChatGPT a safer fallback when its own tool-safety layer refuses an unnecessarily broad execution request before it ever reaches this app.
...
To set it up, open **Chat → Settings**, paste an [OpenRouter API key](https://openrouter.ai/keys) and pick a model. The key is stored with DPAPI like every other credential and never reaches the UI process. The model list is fetched live, filtered to text-capable models and paged; the default is DeepSeek's small fast model, matched against the live catalogue rather than pinned to an id that would go stale. The app reads the provider's advertised context, maximum completion and exact reasoning efforts rather than assuming every reasoning model accepts the same levels. **Off** requests actual no-reasoning where the provider allows it; models that require reasoning disable that choice. Compaction may request up to **64k output tokens**, clamped to the model/provider limit and to the remaining context budget.
...
**Compact & resume** does the same and then opens a fresh ChatGPT tab with a short instruction telling the model to call `resume_session`. The same action is available directly inside ChatGPT's existing **+** composer submenu as **Compact & continue** when the extension is paired. The new chat is opened **only after a complete handoff is safely on disk**; an output truncated by the provider is treated as a failed compaction rather than being saved and resumed as if it were complete. The extension never pastes the large brief into the composer: the fresh chat receives only the short handoff id and pulls the stored brief through the paged `resume_session` MCP tool. `session_history` remains available for a narrow slice of the original recorded events — a specific tool call, a search, a range — when the brief is not enough. The raw history stays available; the brief never replaces it.
...
- Routing, pending messages and swarm state survive app restarts. The extension only opens/manages the fresh chats and types their bootstrap instruction; worker ↔ prime messages themselves stay in the MCP broker. A worker bootstrap is not considered finished merely because its opening message was typed: it stays leased until the worker actually joins, so a transient platform-side refusal of the join tool is retried instead of leaving a permanent `invited` zombie.
